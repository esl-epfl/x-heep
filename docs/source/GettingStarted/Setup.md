...
> [!Note]
> Backward compatibility with Verilator 4.210 is currently maintained, yet _this is very likely to change_ in future releases, so we strongly suggest against using it. Also, Verilator 4.210 _requires GCC 12.0 or older_, so make sure to configure your environment accordingly if you choose to use it anyway.
