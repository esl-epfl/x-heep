 ...
 */
typedef struct rv_timer_tick_params {
  /**
   ...
   */
  uint8_t tick_step;
} rv_timer_tick_params_t;

/**
 * Represents the result of an RV timer operation..
 */
typedef enum rv_timer_result {
  kRvTimerOk = 0,
  kRvTimerBadArg = 1,
  kRvTimerError = 2,
} rv_timer_result_t;
...
/**
 * Errors returned by `rv_timer_approximate_tick_params()`.
 */
typedef enum rv_timer_approximate_tick_params_result {
  kRvTimerApproximateTickParamsOk = kRvTimerOk,
  kRvTimerApproximateTickParamsBadArg = kRvTimerBadArg,
  kRvTimerApproximateTickParamsError = kRvTimerError,
  /**
   * Indicates that the requested counter frequency cannot be represented
   * without loss in precission.
   */
  kRvTimerApproximateTickParamsUnrepresentable,
} rv_timer_approximate_tick_params_result_t;

/**
 * Generates an aproximate `rv_timer_tick_params_t` given the device
...
rv_timer_approximate_tick_params(uint64_t clock_freq, uint64_t counter_freq,
                                     rv_timer_tick_params_t *out);
/**
 * Configuration for initializing the RISC-V timer device.
 */
typedef struct rv_timer_config {
  /**
 ...
 */
typedef struct rv_timer {
  mmio_region_t base_addr;
  rv_timer_config_t config;
} rv_timer_t;
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_init(mmio_region_t base_addr,
                                        rv_timer_config_t config,
                                        rv_timer_t *timer_out);
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_reset(const rv_timer_t *timer);
 ...
 * The function `rv_timer_approximate_tick_params()` can be used to generate
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_set_tick_params(
    const rv_timer_t *timer, uint32_t hart_id,
    rv_timer_tick_params_t params);
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_counter_set_enabled(
    const rv_timer_t *timer, uint32_t hart_id,
    rv_timer_enabled_t state);
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_counter_read(const rv_timer_t *timer,
                                                uint32_t hart_id,
                                                uint64_t *out);
 ...
 * contains a bug:
 *   uint64_t time;
 *   rv_timer_counter_read(my_timer, kMyHart, &time);
 *   time += kSomeDuration;  // (*)
 *   rv_timer_arm(my_timer, kMyHart, kMyComp, time);
 *
 * If `time` wraps around when performing the addition, an interrupt will be
 * fired immediately upon calling `rv_timer_arm`. Care should be taken to
 ...
 * @return The result of the operation.
 */
rv_timer_result_t rv_timer_arm(const rv_timer_t *timer,
 ...
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_enable(const rv_timer_t *timer,
                                              uint32_t hart_id,
                                              uint32_t comp_id,
                                              rv_timer_enabled_t state);
 ...
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_get(const rv_timer_t *timer,
                                           uint32_t hart_id, uint32_t comp_id,
                                           bool *flag_out);
 ...
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_clear(const rv_timer_t *timer,
                                             uint32_t hart_id,
                                             uint32_t comp_id);
 ...
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_force(const rv_timer_t *timer,
                                             uint32_t hart_id,
                                             uint32_t comp_id);
 ...
 * `state` may be NULL. See `rv_timer_irq_restore()`.
 *
 * @param timer a timer device.
 * @param[out] state IRQ state information for use with
 *                   `rv_timer_irq_restore`.
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_disable(const rv_timer_t *timer,
                                               uint32_t hart_id,
                                               uint32_t *state);
 ...
 * @return the result of the operation.
 */
rv_timer_result_t rv_timer_irq_restore(const rv_timer_t *timer,
                                               uint32_t hart_id,
                                               uint32_t state);
