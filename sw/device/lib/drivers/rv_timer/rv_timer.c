...
rv_timer_result_t rv_timer_init(mmio_region_t base_addr,
  ...
  if (timer_out == NULL || config.hart_count < 1 ||
    ...
    return kRvTimerBadArg;
  }

  timer_out->base_addr = base_addr;
  timer_out->config = config;

  return rv_timer_reset(timer_out);
}
...
rv_timer_approximate_tick_params(uint64_t clock_freq, uint64_t counter_freq,
  ...
  if (out == NULL) {
    return kRvTimerApproximateTickParamsBadArg;
  }
  ...
  if (prescale > RV_TIMER_CFG0_PRESCALE_MASK ||
    ...
    return kRvTimerApproximateTickParamsUnrepresentable;
  }

  out->prescale = (uint16_t)prescale;
  out->tick_step = (uint8_t)step;

  return kRvTimerApproximateTickParamsOk;
}
...
rv_timer_result_t rv_timer_reset(const rv_timer_t *timer) {
  ...
  for (uint32_t hart_id = 0; hart_id < timer->config.hart_count; ++hart_id) {
    ...
    for (uint32_t comp_id = 0; comp_id < timer->config.comparator_count;
         ++comp_id) {
      rv_timer_result_t error =
          rv_timer_arm(timer, hart_id, comp_id, UINT64_MAX);
      if (error != kRvTimerOk) {
        return error;
      }
    }
