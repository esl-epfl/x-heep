...
volatile uint8_t spi_get_tx_queue_depth(const spi_t *spi) {
    volatile uint32_t status_reg = spi_get_status(spi);
    return bitfield_field32_read(status_reg, SPI_HOST_STATUS_TXQD_FIELD);
}

volatile spi_ch_status_t spi_get_tx_channel_status(const spi_t *spi) {
    volatile uint32_t status_reg = spi_get_status(spi);
...
volatile uint8_t spi_get_rx_queue_depth(const spi_t *spi) {
    volatile uint32_t status_reg = spi_get_status(spi);
    return bitfield_field32_read(status_reg, SPI_HOST_STATUS_RXQD_FIELD);
}

volatile spi_ch_status_t spi_get_rx_channel_status(const spi_t *spi) {
    volatile uint32_t status_reg = spi_get_status(spi);
    spi_ch_status_t ch_status = {
...
volatile uint32_t spi_get_csid(const spi_t *spi) {
    return mmio_region_read32(spi->base_addr, SPI_HOST_CSID_REG_OFFSET);
}

// SPI set functions

static void spi_sw_reset(const spi_t *spi) {
