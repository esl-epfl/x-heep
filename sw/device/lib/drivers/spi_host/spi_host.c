...
spi_return_flags_e spi_set_configopts(const spi_host_t *spi, uint32_t csid, const uint32_t conf_reg) {
...
spi_return_flags_e spi_set_command(const spi_host_t *spi, const uint32_t cmd_reg) {
    if (spi == NULL) return SPI_FLAG_NULL_PTR;
    if (bitfield_read(spi_get_status(spi), SPI_HOST_STATUS_CMDQD_MASK, SPI_HOST_STATUS_CMDQD_OFFSET) >= SPI_HOST_PARAM_CMD_DEPTH)
