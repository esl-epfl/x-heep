/**
 * @file main.c
 * @brief Simple quad spi write example using BSP
 *
 * Simple example that read some bytes from memory and checks
...
int main(int argc, char *argv[]) {
    ...
    printf("%dB buffer...\n\r", len);
    status = w25q128jw_read_quad(test_buffer, (void*)flash_data, len);
    if (status != FLASH_OK) return EXIT_FAILURE;
