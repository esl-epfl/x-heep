...
int im2col_nchw_int32()
{
        ...
        for (int32_t b = 0; b < BATCH; ++b) {
            ...
            for (int h = 0; h < N_PATCHES_H; ++h) {
                ...
                if (PAD > 0 && im_row < 0)
                {
                    ...
                    PRINTF_DEB("\n\rAdded the initial full row of 0s, %d elements", patches_w);
                } 
                else if (PAD > 0 && im_row >= IW)
                {
                    /* im_row >= IH case: only the output_image_ptr needs to be updated */
                    output_data_ptr += N_PATCHES_W;
                    PRINTF("\n\rAdded the final full row of 0s, %d elements", patches_w);
                }
                else
                {
                    #if TIMING
    ...
    #elif HW_CONFIG == 2
    /* Implementation of an im2col algorithm using the DMA 2D feature */
