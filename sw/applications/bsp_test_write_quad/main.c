/**
 * @file main.c
 * @brief Simple spi quad write example using BSP
 *
 * Write some bytes to memory and checks that the data is correct.
 * Both read and write operations are performed using quad spi.
 *
*/
...
int main(int argc, char *argv[]) {
    ...
    status = w25q128jw_write_quad(FLASH_ADDR, test_buffer, len);
    if (status != FLASH_OK) return EXIT_FAILURE;

    // Read from flash memory at the same address
    status = w25q128jw_read_quad(FLASH_ADDR, flash_data, len);
    if (status != FLASH_OK) return EXIT_FAILURE;
