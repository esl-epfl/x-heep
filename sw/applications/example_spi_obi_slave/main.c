...
spi_slave_flags_e spi_host_write(uint32_t addr, uint8_t *data, uint16_t length);
...
spi_slave_flags_e spi_slave_read(uint32_t addr, void* data, uint16_t length){
    ...
    for(int i = 0; i < 4; i++){ //To test but it might be possible to delete this and put .len in send_cmd_byte to 3. But not sure if that would work as intended
...
static spi_slave_flags_e spi_host_write(uint32_t addr, uint8_t *data, uint16_t length) {
