...
% endfor
  ...
  localparam logic[31:0] PAD_START_ADDRESS = AO_PERIPHERAL_START_ADDRESS + 32'h${pad_start_offset};
  localparam logic[31:0] PAD_SIZE = 32'h${pad_size_address};
  localparam logic[31:0] PAD_END_ADDRESS = PAD_START_ADDRESS + PAD_SIZE;
  localparam logic[31:0] PAD_IDX = 32'd7;
  ...
  localparam addr_map_rule_t [AO_PERIPHERALS-1:0] AO_PERIPHERALS_ADDR_RULES = '{
      ...
      '{ idx: PAD_IDX, start_addr: PAD_START_ADDRESS, end_addr: PAD_END_ADDRESS }
  };
...
% endfor
  ...
  localparam addr_map_rule_t [NUM_PAD-1:0] PAD_ADDR_RULES = '{
      ...
      '{ idx: ${pad.index}, start_addr: (PAD_START_ADDRESS + ${4 * pad.index}), end_addr: (PAD_END_ADDRESS + ${4 * pad.index})},
      % endfor
      '{ idx: ${pad_list[-1].index}, start_addr: (PAD_START_ADDRESS + ${4 * pad_list[-1].index}), end_addr: (PAD_END_ADDRESS + ${4 * pad_list[-1].index})}
  };
