...
initial begin
   ...
   if (fpdm) begin $display("PDM File opened successfully.");
   end else begin $display("Failed to open PDM file.");
   end

   fpcm = $fopen("signals/pcm.txt", "w");
   if (fpcm) begin $display("PCM File opened successfully.");
   end else begin $display("Failed to open PCM file.");
   end
