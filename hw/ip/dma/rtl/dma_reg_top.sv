...
module dma_reg_top #(
  ...
  always_comb begin
    ...
    addr_hit[11] = (reg_addr == DMA_SPC_SLOT_OFFSET);
    addr_hit[12] = (reg_addr == DMA_DATA_TYPE_OFFSET);
    addr_hit[13] = (reg_addr == DMA_MODE_OFFSET);
    addr_hit[14] = (reg_addr == DMA_DIM_CONFIG_OFFSET);
    addr_hit[15] = (reg_addr == DMA_DIM_INV_OFFSET);
    addr_hit[16] = (reg_addr == DMA_PAD_TOP_OFFSET);
    addr_hit[17] = (reg_addr == DMA_PAD_BOTTOM_OFFSET);
    addr_hit[18] = (reg_addr == DMA_PAD_RIGHT_OFFSET);
    addr_hit[19] = (reg_addr == DMA_PAD_LEFT_OFFSET);
    addr_hit[20] = (reg_addr == DMA_WINDOW_SIZE_OFFSET);
    addr_hit[21] = (reg_addr == DMA_WINDOW_COUNT_OFFSET);
    addr_hit[22] = (reg_addr == DMA_INTERRUPT_EN_OFFSET);
    addr_hit[23] = (reg_addr == DMA_TRANSACTION_IFR_OFFSET);
    addr_hit[24] = (reg_addr == DMA_WINDOW_IFR_OFFSET);
  end
  ...
  assign spc_slot_rx_trigger_slot_we = addr_hit[11] & reg_we & !reg_error;
  assign spc_slot_rx_trigger_slot_wd = reg_wdata[3:0];

  assign spc_slot_tx_trigger_slot_we = addr_hit[11] & reg_we & !reg_error;
  assign spc_slot_tx_trigger_slot_wd = reg_wdata[7:4];

  assign data_type_we = addr_hit[12] & reg_we & !reg_error;
  ...
  assign window_size_we = addr_hit[20] & reg_we & !reg_error;
  assign window_size_wd = reg_wdata[12:0];

  assign interrupt_en_transaction_done_we = addr_hit[22] & reg_we & !reg_error;
  assign interrupt_en_transaction_done_wd = reg_wdata[0];

  assign interrupt_en_window_done_we = addr_hit[22] & reg_we & !reg_error;
  assign interrupt_en_window_done_wd = reg_wdata[1];

  assign transaction_ifr_re = addr_hit[23] & reg_re & !reg_error;

  assign window_ifr_re = addr_hit[24] & reg_re & !reg_error;
